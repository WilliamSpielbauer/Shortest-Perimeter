...
def checkBounds(p1,p2,points,addMeta = False):
    ...
    try:
        ...
        for px, py in points:
            if px == p1[0] and py == p1[1]:
                continue
            elif px == p2[0] and py == p2[1]:
                continue
...
def deform(perimeter, pos, points):
    ...
    while(len(perimeter)-1 != pos):
        ...
        # If a point in the perimeter is found in outOfBounds, remove it from outOfBounds 
        outOfBounds = [x for x in outOfBounds if x not in perimeter] 
        ...
        if outOfBounds == []:
            ...
            if type(vals) is int:
                ...
                try:
                    ...
                    # line from point to function line (perpendicular line) = -a^(-1)x + b
                    a,c = vals
                    for x,y in outOfBounds:
                        b = y + (a**-1)*x
                        # extract distance 
                        ...
                        distance = abs(distanceX-x)**2 + abs(distanceY-y)**2    # no need to square root: only need to see which is largest
